...
void NormalEnemy::Initialize(const uint32_t& modelHandle, const Vector3& position, const Vector3& speed) {
	...
	//モデルの生成
	animationmodel_.reset(Kamaboko::AnimationModel::Create(modelHandle));

	//スキニングアニメーション読み込み
	skeleton_.Create(Kamaboko::ModelManager::GetInstance()->GetModelData(modelHandle).rootNode);
	skinCluster_.Create(skeleton_, Kamaboko::ModelManager::GetInstance()->GetModelData(modelHandle));
	animationHandle_ = Kamaboko::AnimationManager::GetInstance()->LoadFile("Resources/LevelData/GameStage/Ghost","Ghost.gltf");

	//感電パーティクル用
	thunderTextureHandle_ = Kamaboko::TextureManager::Load("Resources/Sprite/Particle/Thunder.png");
	...
	//デバッグ用のモデル
	debugModelHandle = Kamaboko::ModelManager::GetInstance()->Load("Resources/Model/Sample/Sphere", "Sphere.obj");
...
void NormalEnemy::Update() {
	...
	//アニメーション適用
	Kamaboko::AnimationManager::GetInstance()->ApplyAnimation(skeleton_, animationHandle_, animationTime_);
...
void NormalEnemy::Damaged() {
	...
	//瀕死
		...
		if (electricShockParticle_ == nullptr) {
			//生成
			electricShockParticle_ = std::move(Kamaboko::Particle3D::Create(ParticleMoveType::Stay));
			//テクスチャの上書き
...
void NormalEnemy::Delete() {
	...
	if (deadParticle_ == nullptr) {
		//生成
		deadParticle_ = std::move(Kamaboko::Particle3D::Create(ParticleMoveType::Rise));
		//パーティクルの細かい設定
