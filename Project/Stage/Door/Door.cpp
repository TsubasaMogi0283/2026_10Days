...
void Door::Initialize() {
	...
	//テクスチャの読み込み
	uint32_t textureHandle = Kamaboko::TextureManager::GetInstance()->Load("Resources/Sprite/Open/Open.png");

	//生成
	openSptrite_ .reset(Kamaboko::Sprite::Create(textureHandle, INITIAL_POSITION));
	//最初は非表示
	...
	//開ける音の読み込み
	openSEHandle_ = Kamaboko::Audio::GetInstance()->Load("Resources/Audio/Action/OpenDoor.mp3");
}
...
void Door::Update() {
	...
	if (distance_ < MAX_DISTANCE_ && isOpen_ == false) {
		...
		if (input_->IsTriggerKey(DIK_SPACE) == true || input_->IsTriggerButton(XINPUT_GAMEPAD_B) == true) {
			//開ける音を鳴らす
			Kamaboko::Audio::GetInstance()->Play(openSEHandle_, false);
			isOpen_ = true;
		}
	}
	else {
