...
void KeyManager::Initialize(const uint32_t& modelHandle, const std::vector<Vector3>& positions) {
	...
	//生成
	keyListSprite_.reset(Kamaboko::Sprite::Create(keyListSpriteHandle, initialPosition_));

	uint32_t textureHandle = Kamaboko::TextureManager::GetInstance()->Load("Resources/Sprite/Item/Key.png");
	//サイズを取得
	...
	for (uint32_t i = 0u; i < MAX_KEY_QUANTITY_; ++i) {
		...
		keySprites_[i].reset(Kamaboko::Sprite::Create(textureHandle, position));
		//アンカーポイントの設定
	...
	pickUpKey_.reset(Kamaboko::Sprite::Create(pickUpTextureManager, INITIAL_FADE_POSITION));
