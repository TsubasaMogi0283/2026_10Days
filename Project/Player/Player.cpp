...
void Player::Initialize(){
	...
	for (uint32_t i = 0u; i < PLAYER_HP_MAX_QUANTITY_; ++i) {
		playerHpSprite_[i].reset(Kamaboko::Sprite::Create(playerHPTextureHandle, { .x = static_cast<float_t>(i) * 64 + FRAME_INITIAL_POSITION.x,.y = FRAME_INITIAL_POSITION.y }));
	}
	//フレームを生成
	playerHPBackFrameSprite_.reset(Kamaboko::Sprite::Create(playerHPBackFrameTextureHandle, FRAME_INITIAL_POSITION));
