...
void FlashLight::Initialize() {
	...
	attackWhiteFadeSprite_.reset(Kamaboko::Sprite::Create(whiteTextureHandle, FADE_INITIAL_POSITION));
	//初期の透明度を設定
	...
	chargeGaugeSprite_.reset(Kamaboko::Sprite::Create(gaugeTextureHandle, chargeGaugeSpritePosition_));
	//チャージの増減値
	...
	frameSprite_.reset(Kamaboko::Sprite::Create(frameSpriteHandle, chargeGaugeSpritePosition_));
	//当たり判定の初期化
	...
	//デバッグ用のモデルを生成する
	uint32_t debugModelHandle = Kamaboko::ModelManager::GetInstance()->Load("Resources/Model/Sample/Sphere", "Sphere.obj");

	const float SCALE = 0.4f;
	//左右
	for (uint32_t i = 0; i < SIDE_QUANTITY_; ++i) {
		sideModel_[i].reset(Kamaboko::Model::Create(debugModelHandle));
	...
	//中心
	lightCenterModel_.reset(Kamaboko::Model::Create(debugModelHandle));
...
void FlashLight::DrawObject3D(const Camera& camera) {
	...
	for (const std::unique_ptr <Kamaboko::Particle3D>& particle : chargeParticle_) {
		if (particle != nullptr) {
...
void FlashLight::GenerateParticle() {
	//生成
	std::unique_ptr<Kamaboko::Particle3D> particle = Kamaboko::Particle3D::Create(ParticleMoveType::Absorb);
