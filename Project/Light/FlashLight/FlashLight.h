...
private:
	//グローバル変数クラス
	Kamaboko::GlobalVariables* globalVariables_ = nullptr;
	//テクスチャ管理クラス
...
private:
	...
	//チャージ時のパーティクル
	std::list<std::unique_ptr<Kamaboko::Particle3D>>chargeParticle_ = {};
	//パーティクルマテリアル
	...
	//チャージのゲージ
	std::unique_ptr<Kamaboko::Sprite>chargeGaugeSprite_ = nullptr;
	//通常のチャージの色
	Vector4 chargeColor_ = {};

	//フレーム
	std::unique_ptr<Kamaboko::Sprite>frameSprite_ = nullptr;
	//チャージしているかどうか
	...
	//攻撃のホワイトフェード
	std::unique_ptr<Kamaboko::Sprite>attackWhiteFadeSprite_ = nullptr;
...
private:
	...
	std::unique_ptr<Kamaboko::Model>sideModel_[SIDE_QUANTITY_] = { nullptr };
	WorldTransform worldTransform_[SIDE_QUANTITY_] = {};
	Material material_ = {};

	//中心
	std::unique_ptr<Kamaboko::Model>lightCenterModel_ = nullptr;
