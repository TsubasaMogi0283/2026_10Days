...
void ExplanationGameScene::Initialize(){
	...
	for (uint32_t i = 0u; i < explanation_.size(); ++i) {
		explanation_[i].reset(Kamaboko::Sprite::Create(explanationTextureHandle[i], INITIAL_SPRITE_POSITION_));
	}
	...
	for (uint32_t i = 0u; i < spaceToNext_.size(); ++i) {
		spaceToNext_[i].reset(Kamaboko::Sprite::Create(spaceToNextTextureHandle[i], INITIAL_SPRITE_POSITION_));
	}
