...
void PlayGameScene::Initialize(){
	...
	//コリジョン管理クラスの生成
	collisionManager_ = std::make_unique<Kamaboko::CollisionManager>();

	//操作
	uint32_t operationTextureHandle = textureManager_->Load("Resources/Sprite/Operation/Operation.png");
	//生成
	operationSprite_.reset(Kamaboko::Sprite::Create(operationTextureHandle, { .x = 20.0f,.y = 0.0f }));
	//出口の画像読み込み
	uint32_t escapeTextureHandle = textureManager_->Load("Resources/Sprite/Escape/EscapeText.png");
	//生成
	escapeTextSprite_.reset(Kamaboko::Sprite::Create(escapeTextureHandle, INITIAL_SPRITE_POSITION_));
	//最初は非表示にする
	...
	//生成
	blackFadeSprite_.reset(Kamaboko::Sprite::Create(blackTextureHandle, INITIAL_SPRITE_POSITION_));
	...
	toEscapeSprite_.reset(Kamaboko::Sprite::Create(toEscapeTextureHandle, INITIAL_SPRITE_POSITION_));
	
	//矢印のモデルを読み込む
	uint32_t arrowModelHandle = Kamaboko::ModelManager::GetInstance()->Load("Resources/Model/Arrow","Arrow.obj");
	//門の中心座標を計算
	...
	//ゲートのモデルの読み込み
	uint32_t gateModelhandle = Kamaboko::ModelManager::GetInstance()->Load("Resources/Model/Sample/Gate", "Gate.obj");
	//生成
...
void PlayGameScene::Update(GameScene* gameScene){
	...
	if (isSucceedEscape_ == true) {
		...
		if (isPlayOpenSE_ == false) {
			Kamaboko::Audio::GetInstance()->Play(openGateAudioHandle_, false);
			isPlayOpenSE_ = true;
		}
	...
	//取っていないかつ墓場にいたら柵が下がり閉じ込められる
		...
		if (player_->GetWorldPosition().z <= -26.0f &&
			...
			if (isPlayCloseSE_ == false) {
				...
				//閉まる音を再生
				Kamaboko::Audio::GetInstance()->Play(closeGateAudioHandle_, false);
				isPlayCloseSE_ = true;
			}
